...
import { redactImage, formatFileSize, copyImageToClipboard } from '../../utils/imageProcessor';
...
const RedactImage = () => {
    ...
    const handleProcess = useCallback(async () => {
        ...
        try {
            const blob = await redactImage(file, blurAmount, setProgress);
    ...
    const handleDownload = useCallback(() => {
        ...
        link.download = `${baseName}_redacted.jpg`;
        link.click();
    }, [result, file]);
    ...
    const schemaMarkup = {
        ...
        "description": t('pages.image.redactImage.hero.description')
    };
    ...
    return (
        <div className="background-blur">
            ...
            <ToolBreadcrumb
                currentLabel="Redact Image"
                guidePath="/guide/blur-background"
            />

            <div className="page-header">
                <h1>{t('pages.image.redactImage.hero.title')}</h1>
                <p>{t('pages.image.redactImage.hero.description')}</p>
            </div>
            ...
            {!file && (
                ...
                >
                    ...
                    <label htmlFor="file-input" className="upload-content">
                        ...
                        <p>{t('pages.image.redactImage.upload.hint')}</p>
                        <span className="upload-hint">{t('pages.image.redactImage.upload.support')}</span>
                    </label>
                </div>
            )}
                ...
                <div className="editor">
                    <div className="image-compare">
                        <div className="image-panel">
                            <h3>{t('pages.image.redactImage.panels.original')}</h3>
                            <div
                        ...
                        <div className="image-panel">
                            <h3>{t('pages.image.redactImage.panels.result')}</h3>
                            <div
                                ...
                                {result ? (
                                    ...
                                    <div className="placeholder">
                                        {processing
                                            ...
                                            : t('pages.image.redactImage.placeholders.result')}
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    <div className="options">
                        <div className="option-group">
                            <label>{t('pages.image.redactImage.options.blurStrength')} ({blurAmount})</label>
                            <input
                                type="range"
                                min="1"
                                max="50"
                            ...
                            <div className="range-labels">
                                <span>{t('pages.image.redactImage.options.rangeSoft')}</span>
                                <span>{t('pages.image.redactImage.options.rangeStrong')}</span>
                            </div>
                        </div>
                    ...
                    {processing && (
                        <div className="progress" aria-live="polite">
                            <p>{t('pages.image.redactImage.progress.label')}</p>
                    ...
                    <div className="actions">
                        ...
                        {!result ? (
                            <button
                                ...
                                disabled={processing}
                            >
                                {processing ? t('common.status.processing') : t('pages.image.redactImage.actions.apply')}
                            </button>
                        ) : (
                            <>
                                <button
