...
### ✅ 실행 계획: 정책 페이지 & 푸터 링크
1. **현황 점검**
   - 기존 라우트/컴포넌트에 `/privacy-policy`, `/terms`, `/licenses` 페이지가 없는지 확인하고, 비슷한 정적 페이지 컴포넌트가 있다면 재활용 가능 여부 파악.
   - Footer 컴포넌트 구조/스타일을 확인해 새 링크를 추가해도 레이아웃이 깨지지 않는지 검토.
2. **콘텐츠 정의 & 번역 키 설계**
   - 개인정보 처리, 이용약관, 오픈소스 라이선스 초안을 Markdown 형태로 정리하고 `lokit_lang` 기준으로 ko/en 번역 키를 locales에 추가.
   - 공통 서론/섹션 제목은 재사용 가능한 키로 분리하여 유지보수성을 확보.
3. **페이지 컴포넌트 구현**
   - `src/pages/policy` 등 하위 디렉터리에 세 페이지를 생성하고 Hub/PdfHome과 동일한 레이아웃 래퍼를 활용.
   - `<MetaUpdater>`에 route별 title/description/og 정보 추가, `<main>` 구조 내 heading 계층 (h1→h2) 유지.
4. **라우팅 & 네비게이션 연결**
   - React Router에 세 경로를 등록하고 lazy-load 가능하면 동일 패턴 적용.
   - Footer/기타 약관 링크가 필요한 컴포넌트에서 새 라우트로 연결.
5. **검증**
   - 데스크톱/모바일에서 레이아웃 깨짐 여부 확인.
   - 다국어 토글 시 해당 페이지 텍스트가 올바르게 변환되는지 확인.
   - 빌드(`npm run build`)로 타입/린트 오류 없는지 확인 후 배포 체크리스트에 정책 페이지 포함.
